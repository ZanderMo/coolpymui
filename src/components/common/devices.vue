...
<div class="hubManage" :id="'mBtn'+Device.Id">
    <button class="btn btn-info btn-lg" @click="showManageBox('editHub',Device)">修改枢纽</button><button class="btn btn-warning btn-lg"
        @click="showManageBox('addDevice',Device)">添加节点</button><button class="btn btn-danger btn-lg" @click="showManageBox('delHub',Device)">删除枢纽</button>
</div>
...
<group >
    <x-switch :title="item.Title+' :'" style="margin-top:0px;"></x-switch>
</group>
</div>
...
<span class="input-group-addon" style="padding:0px;"><button style="border: 0;background-color:#f0ad4e;height: 32px;width: 50px;color:#fff">发 送</button>
</span>
</div>
<div v-if="item.Type===3" class="deviceItem">{{item.Title}}: LED-可调灯 当前值：{{item.CtrlerVal}}
...
<div>
    <ul>
        <li v-for="(tag,index) in manageHubEv.HubTags">{{tag}}<a @click="deleteTag(index)" id="tag"> X</a></li>
    </ul>
...
<div>
    <ul>
        <li v-for="(tag,index) in manageHubEv.DeviceTags">{{tag}}<a @click="deleteTag(index)" id="tag"> X</a></li>
    </ul>
    <div style="clear:both"></div>
</div>
</div>
...
<div class="addHub-box">
    <a @click="showManageBox('addHub','0')">
        <h4 class="addHub-font"> 添加枢纽</h4>
